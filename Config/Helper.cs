...
public class Helper
{
    ...
    private static async Task<object> BuildEmbedPayload(int mode, string message, string steamId64, string steamName, string serverIpPort)
    {
        var color = GetColorFromConfig();
        var profileLink = $"https://steamcommunity.com/profiles/{steamId64}";
        var avatar = await GetProfilePictureAsync(steamId64, Configs.GetConfigData().Discord_UsersWithNoAvatarImage);
        var formatted = $"```{message}```";

        var kyiv = TimeZoneInfo.FindSystemTimeZoneById("FLE Standard Time");
        var kt = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, kyiv).ToString("HH:mm");

        var embed = new
        {
            type = "rich",
            color,
            author = new { name = steamName, icon_url = avatar, url = profileLink },
            description = formatted,
            fields = new[]
            {
                new { name = "🆔 SteamId", value = "```"+steamId64+"```", inline = true },
                new { name = "🧾 Профиль", value = $"[`Клик`]({profileLink})", inline = true }
            },
            footer = new { text = $"IP: {serverIpPort} • Час: {kt}" }
        };
    ...
    public static void LogLocally(CCSPlayerController player, string message, bool TeamChat)
    {
        ...
        string Fpath = Path.Combine(Configs.Shared.CookiesModule!,"../../plugins/Chat-Logger-ShadowRipper/logs/");
        string fileName = DateTime.Now.ToString(Configs.GetConfigData().Locally_DateFormat) + ".txt";
        string Tpath = Path.Combine(Configs.Shared.CookiesModule!,"../../plugins/Chat-Logger-ShadowRipper/logs/") + $"{fileName}";
